...
from fibo.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from fibo.protocols import wallet_protocol
from fibo.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from fibo.protocols.protocol_message_types import ProtocolMessageTypes
from fibo.protocols.wallet_protocol import (
...
from fibo.server.node_discovery import WalletPeers
...
from fibo.util.errors import Err, ValidationError
from fibo.util.ints import uint32, uint128
from fibo.util.keychain import Keychain
from fibo.util.lru_cache import LRUCache
from fibo.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from fibo.wallet.settings.settings_objects import BackupInitialized
...
from fibo.wallet.wallet_blockchain import ReceiveBlockResult
from fibo.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'fibo keys' program.")
            return None
