...
from fibo.ssl.create_ssl import generate_ca_signed_cert, get_chia_ca_crt_key, make_ca_cert
from fibo.util.bech32m import encode_puzzle_hash
from fibo.util.config import (
...
from fibo.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'fibo keys generate'")
        return None
...
def chia_init(root_path: Path):
    if os.environ.get("CHIA_ROOT", None) is not None:
        print(
            ...
            f"Please unset the environment variable and run fibo init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Chia directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run fibo init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'fibo keys show --show-mnemonic-seed'")
