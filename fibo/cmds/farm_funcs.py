...
from fibo.cmds.units import units
from fibo.consensus.block_record import BlockRecord
from fibo.rpc.farmer_rpc_client import FarmerRpcClient
from fibo.rpc.full_node_rpc_client import FullNodeRpcClient
from fibo.rpc.wallet_rpc_client import WalletRpcClient
from fibo.util.config import load_config
from fibo.util.default_root import DEFAULT_ROOT_PATH
from fibo.util.ints import uint16
from fibo.util.misc import format_bytes
from fibo.util.misc import format_minutes
from fibo.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total fibo farmed: {amounts['farmed_amount'] / units['fibo']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['fibo']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['fibo']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'fibo start wallet' and 'fibo wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'fibo wallet show'")
    else:
        print("Note: log into your key using 'fibo wallet show' to see rewards for each key")
